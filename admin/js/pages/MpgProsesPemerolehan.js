function MpgProsesPemerolehan () {
  
  const className = 'MpgProsesPemerolehan';
  let self = this;
  let dtDisplay;
  let modalConfirmDelete;
  let modalConfirmAction;
  let modalMpgProsesPemerolehan;
  ...
  let vData = [
    {
      field_id: "txtPrpmnoPemerolehan",
      type: "text",
      name: "No Pemerolehan",
      validator: {}
    },
    {
      field_id: "txtPrpmSkim",
      type: "text",
      name: "Skim",
      validator: {}
    },
    {
      field_id: "txtPrpmSuruhanjaya",
      type: "text",
      name: "Suruhanjaya",
      validator: {}
    },
    {
      field_id: "txtPrpmTarikhDari",
      type: "text",
      name: "Tarikh Mesyuarat (Dari)",
      validator: {}
    },
    {
      field_id: "txtPrpmTarikhTutup",
      type: "text",
      name: "Tarikh (Tutup)",
      validator: {}
    },
    {
      field_id: "optPrpmStatus",
      type: "select",
      name: "Status",
      validator: {}
    }
  ...
  this.init = function () {
    try {
      ...
      $('#btnPrpmFilter').on('click', function () {
        if (!formValidate.validateNow()) {
          toastr['error'](_ALERT_MSG_VALIDATION, _ALERT_TITLE_ERROR);
        } else {
          dtPpl.search('').columns().search('').draw();
          if ($('#txtPrpmnoPemerolehan').val().length > 0) {
            dtPpl.columns(1).search($('#txtPrpmnoPemerolehan').val() ? $('#txtPrpmnoPemerolehan').val() : '', false, true, false);
          }
          if ($('#txtPrpmSkim').val().length > 0) {
            dtPpl.columns(1).search($('#txtPrpmSkim').val() ? $('#txtPrpmSkim').val() : '', false, true, false);
          }
          if ($('#txtPrpmSuruhanjaya').val().length > 0) {
            dtPpl.columns(1).search($('#txtPrpmSuruhanjaya').val() ? $('#txtPrpmSuruhanjaya').val() : '', false, true, false);
          }
          if ($('#txtPrpmTarikhDari').val().length > 0) {
            moment.locale('ms');
            const tarikhDari = moment($('#txtPrpmTarikhDari').val(), 'DD/MM/YYYY').format('YYYY-MM-DD');
            // dtPpl.columns(6).search($('#txtPrpmTarikhDari').val() ? tarikhDari : '', false, true, false);
            console.log(tarikhDari);
            dtPpl.columns(6).search((d) => d >= tarikhDari);
          }
          if ($('#txtPrpmTarikhTutup').val().length > 0) {
            moment.locale('ms');
            const tarikhDari = moment($('#txtPrpmTarikhTutup').val(), 'DD/MM/YYYY').format('YYYY-MM-DD');
            // dtPpl.columns(6).search($('#txtPrpmTarikhTutup').val() ? tarikhDari : '', false, true, false);
            console.log(tarikhDari);
            dtPpl.columns(6).search((d) => d >= tarikhDari);
          }
          dtPpl.draw();
      ...
      $('#btnPrpmClearFilter').on('click', function () {
        formValidate.clearValidation();
        $('#lblPrpmTarikhDari').addClass('active');
        $('#lblPrpmTarikhHingga').addClass('active');
        dtPpl.search('').columns().search('').draw();
      });
      ...
      dtPpl = $('#dtPpl').DataTable({
        ...
        drawCallback: function () {
          ...
          $('.lnkMplView').off('click').on('click', function () {
            ...
            modalMpgProsesPemerolehan.view(123);
          });
          $('.lnkMplEdit').off('click').on('click', function () {
            modalMpgProsesPemerolehan2.edit(123);
          });
        ...
        aoColumns: [
          ...
          { mData: 'statusId', mRender: function (data, type, row, meta) { 
            if (data == 1) {
              return "DALAM PROSES PENGESAHAN";
            } else if (data == 2) {
              return "SAH TAPISAN";
            }  else {
              return "";
          ...
          { mData: 'statusId', mRender: function (data, type, row, meta) { 
            return dtDisplay.getActionProsesPemerolehan('edit', 'lnkMpl', meta.row, data, row['keputusanId']); 
          }},
  ...
  this.setModalMpgProsesPemerolehan = function (_modalMpgProsesPemerolehan) {
    modalMpgProsesPemerolehan = _modalMpgProsesPemerolehan;
  };
