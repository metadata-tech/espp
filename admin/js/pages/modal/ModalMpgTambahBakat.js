function ModalMpgTambahBakat () {
  
  const className = 'ModalMpgTambahBakat';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';
  ...
  this.init = function () {
    ...
    $('#btnMmcpClose').on('click', function () {
      console.log('close');
      // $('#modalMpgTambahBakat').modal('hide');
      // worker.terminate();
    });
  ...
  this.add = function () {
    try {
      ...
      setTimeout(function () {
        ...
        $('#modalMpgTambahBakat').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
