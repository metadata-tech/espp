function ModalMpgTambahRekacipta () {
  
  const className = 'ModalMpgTambahRekacipta';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMrni;
  this.getValidationDataMrni = function () {
    return [
      {
        field_id: "optMrniJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMrni = new MzValidate(false);
    formValidateMrni.registerFields(self.getValidationDataMrni());
    
    $('#btnMrniClose').on('click', function () {
      console.log('close');
      // $('#modalMpgTambahRekacipta').modal('hide');
      // worker.terminate();
    });
      
    $('#optMrniJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMrni.enableField('optMrniCawangan');
        formValidateMrni.enableField('optMrniFungsi');
        formValidateMrni.enableField('optMrniSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMrni.disableField('optMrniCawangan');
        formValidateMrni.disableField('optMrniFungsi');
        formValidateMrni.disableField('optMrniSubFungsi');
      }
    });

    $('#txtMrniDokumen').on('change', function () {
      const preview = document.getElementById('pdfMrniPreview');
      const file = document.getElementById('txtMrniDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMrniIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMrniIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
  ...
  this.add = function () {
    try {
      ...
      formValidateMrni.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MrniTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgTambahRekacipta').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MrniTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>KEMASKINI CATATAN PEMEROLEHAN');
        $('#modalMpgTambahRekacipta').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MrniTitle').html('<i class="fa-duotone fa-clock-rotate-left mr-2"></i>Sejarah Transaksi');
        $('#modalMpgTambahRekacipta').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMrniJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMrniNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMrniTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMrniFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMrniPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMrniIsiDokumen', isiDokumen, 'textarea');
  };
