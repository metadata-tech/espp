function ModalMpgSejarahTransaksi () {
  
  const className = 'ModalMpgSejarahTransaksi';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMmcp;
  this.getValidationDataMmcp = function () {
    return [
      {
        field_id: "optMmcpJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMmcp = new MzValidate(false);
    formValidateMmcp.registerFields(self.getValidationDataMmcp());
    
    $('#btnMmcpClose').on('click', function () {
      console.log('close');
      // $('#modalMpgSejarahTransaksi').modal('hide');
      // worker.terminate();
    });
      
    $('#optMmcpJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMmcp.enableField('optMmcpCawangan');
        formValidateMmcp.enableField('optMmcpFungsi');
        formValidateMmcp.enableField('optMmcpSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMmcp.disableField('optMmcpCawangan');
        formValidateMmcp.disableField('optMmcpFungsi');
        formValidateMmcp.disableField('optMmcpSubFungsi');
      }
    });

    $('#txtMmcpDokumen').on('change', function () {
      const preview = document.getElementById('pdfMmcpPreview');
      const file = document.getElementById('txtMmcpDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMmcpIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMmcpIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
  ...
  this.add = function () {
    try {
      ...
      formValidateMmcp.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MmcpTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgSejarahTransaksi').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MmcpTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>KEMASKINI CATATAN PEMEROLEHAN');
        $('#modalMpgSejarahTransaksi').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MmcpTitle').html('<i class="fa-duotone fa-clock-rotate-left mr-2"></i>Sejarah Transaksi');
        $('#modalMpgSejarahTransaksi').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMmcpJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMmcpNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMmcpTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMmcpFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMmcpPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMmcpIsiDokumen', isiDokumen, 'textarea');
  };
