function ModalMpgProsesPemerolehan () {
  
  const className = 'ModalMpgProsesPemerolehan';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';
  ...
  this.init = function () {
    ...
    $('#btnMpplClose').on('click', function () {
      console.log('close');
      // $('#modalMpgProsesPemerolehan').modal('hide');
      // worker.terminate();
    });
  ...
  this.add = function () {
    try {
      ...
      setTimeout(function () {
        ...
        $('#modalMpgProsesPemerolehan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
