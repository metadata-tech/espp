function ModalMpgTambahPenguasaanBahasa () {
  
  const className = 'ModalMpgTambahPenguasaanBahasa';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMtbp;
  this.getValidationDataMtbp = function () {
    return [
      {
        field_id: "optMtbpJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMtbp = new MzValidate(false);
    formValidateMtbp.registerFields(self.getValidationDataMtbp());
    
    $('#btnMtbpClose').on('click', function () {
      console.log('close');
      // $('#modalMpgTambahPenguasaanBahasa').modal('hide');
      // worker.terminate();
    });
      
    $('#optMtbpJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMtbp.enableField('optMtbpCawangan');
        formValidateMtbp.enableField('optMtbpFungsi');
        formValidateMtbp.enableField('optMtbpSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMtbp.disableField('optMtbpCawangan');
        formValidateMtbp.disableField('optMtbpFungsi');
        formValidateMtbp.disableField('optMtbpSubFungsi');
      }
    });

    $('#txtMtbpDokumen').on('change', function () {
      const preview = document.getElementById('pdfMtbpPreview');
      const file = document.getElementById('txtMtbpDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMtbpIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMtbpIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
  ...
  this.add = function () {
    try {
      ...
      formValidateMtbp.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MtbpTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgTambahPenguasaanBahasa').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MtbpTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>KEMASKINI CATATAN PEMEROLEHAN');
        $('#modalMpgTambahPenguasaanBahasa').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MtbpTitle').html('<i class="fa-duotone fa-clock-rotate-left mr-2"></i>Sejarah Transaksi');
        $('#modalMpgTambahPenguasaanBahasa').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMtbpJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMtbpNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMtbpTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMtbpFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMtbpPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMtbpIsiDokumen', isiDokumen, 'textarea');
  };
