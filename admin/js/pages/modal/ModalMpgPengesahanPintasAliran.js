function ModalMpgPengesahanPintasAliran () {
  
  const className = 'ModalMpgPengesahanPintasAliran';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMppl;
  this.getValidationDataMppl = function () {
    return [
      {
        field_id: "optMpplJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMppl = new MzValidate(false);
    formValidateMppl.registerFields(self.getValidationDataMppl());
    
    $('#btnMpplClose').on('click', function () {
      console.log('close');
      // $('#modalMpgPengesahanPintasAliran').modal('hide');
      // worker.terminate();
    });
      
    $('#optMpplJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMppl.enableField('optMpplCawangan');
        formValidateMppl.enableField('optMpplFungsi');
        formValidateMppl.enableField('optMpplSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMppl.disableField('optMpplCawangan');
        formValidateMppl.disableField('optMpplFungsi');
        formValidateMppl.disableField('optMpplSubFungsi');
      }
    });

    $('#txtMpplDokumen').on('change', function () {
      const preview = document.getElementById('pdfMpplPreview');
      const file = document.getElementById('txtMpplDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMpplIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMpplIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
    ...
    $('#btnMpplSave').on('click', function () {
      if (!formValidateMppl.validateNow()) {
  ...
  this.add = function () {
    try {
      ...
      formValidateMppl.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MpplTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgPengesahanPintasAliran').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MpplTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgPengesahanPintasAliran').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MpplTitle').html('Maklumat Proses Pemerolehan');
        $('#modalMpgPengesahanPintasAliran').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMpplJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMpplNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMpplTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMpplFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMpplPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMpplIsiDokumen', isiDokumen, 'textarea');
  };
