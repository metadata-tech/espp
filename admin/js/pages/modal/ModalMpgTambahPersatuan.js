function ModalMpgTambahPersatuan () {
  
  const className = 'ModalMpgTambahPersatuan';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMpsn;
  this.getValidationDataMpsn = function () {
    return [
      {
        field_id: "optMpsnJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMpsn = new MzValidate(false);
    formValidateMpsn.registerFields(self.getValidationDataMpsn());
    
    $('#btnMpsnClose').on('click', function () {
      console.log('close');
      // $('#modalMpgTambahPersatuan').modal('hide');
      // worker.terminate();
    });
      
    $('#optMpsnJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMpsn.enableField('optMpsnCawangan');
        formValidateMpsn.enableField('optMpsnFungsi');
        formValidateMpsn.enableField('optMpsnSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMpsn.disableField('optMpsnCawangan');
        formValidateMpsn.disableField('optMpsnFungsi');
        formValidateMpsn.disableField('optMpsnSubFungsi');
      }
    });

    $('#txtMpsnDokumen').on('change', function () {
      const preview = document.getElementById('pdfMpsnPreview');
      const file = document.getElementById('txtMpsnDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMpsnIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMpsnIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
  ...
  this.add = function () {
    try {
      ...
      formValidateMpsn.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MpsnTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgTambahPersatuan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MpsnTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>KEMASKINI CATATAN PEMEROLEHAN');
        $('#modalMpgTambahPersatuan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MpsnTitle').html('<i class="fa-duotone fa-clock-rotate-left mr-2"></i>Sejarah Transaksi');
        $('#modalMpgTambahPersatuan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMpsnJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMpsnNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMpsnTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMpsnFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMpsnPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMpsnIsiDokumen', isiDokumen, 'textarea');
  };
