function ModalMpgTambahSukan () {
  
  const className = 'ModalMpgTambahSukan';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMmsk;
  this.getValidationDataMmsk = function () {
    return [
      {
        field_id: "optMmskJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMmsk = new MzValidate(false);
    formValidateMmsk.registerFields(self.getValidationDataMmsk());
    
    $('#btnMmskClose').on('click', function () {
      console.log('close');
      // $('#modalMpgTambahSukan').modal('hide');
      // worker.terminate();
    });
      
    $('#optMmskJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMmsk.enableField('optMmskCawangan');
        formValidateMmsk.enableField('optMmskFungsi');
        formValidateMmsk.enableField('optMmskSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMmsk.disableField('optMmskCawangan');
        formValidateMmsk.disableField('optMmskFungsi');
        formValidateMmsk.disableField('optMmskSubFungsi');
      }
    });

    $('#txtMmskDokumen').on('change', function () {
      const preview = document.getElementById('pdfMmskPreview');
      const file = document.getElementById('txtMmskDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMmskIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMmskIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
  ...
  this.add = function () {
    try {
      ...
      formValidateMmsk.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MmskTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgTambahSukan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MmskTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>KEMASKINI CATATAN PEMEROLEHAN');
        $('#modalMpgTambahSukan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MmskTitle').html('<i class="fa-duotone fa-clock-rotate-left mr-2"></i>Sejarah Transaksi');
        $('#modalMpgTambahSukan').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMmskJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMmskNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMmskTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMmskFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMmskPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMmskIsiDokumen', isiDokumen, 'textarea');
  };
