function ModalMpgProsesPemerolehan2 () {
  
  const className = 'ModalMpgProsesPemerolehan2';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMppl2;
  this.getValidationDataMppl2 = function () {
    return [
      {
        field_id: "optMppl2JenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
      ...
      {
        field_id: "txtMppl2NoRujukanFail",
        type: "text",
  ...
  this.init = function () {
    formValidateMppl2 = new MzValidate(false);
    formValidateMppl2.registerFields(self.getValidationDataMppl2());
    
    $('#btnMppl2Close').on('click', function () {
      console.log('close');
      // $('#modalMpgProsesPemerolehan2').modal('hide');
      // worker.terminate();
    });
      
    $('#optMppl2JenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMppl2.enableField('optMppl2Cawangan');
        formValidateMppl2.enableField('optMppl2Fungsi');
        formValidateMppl2.enableField('optMppl2SubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMppl2.disableField('optMppl2Cawangan');
        formValidateMppl2.disableField('optMppl2Fungsi');
        formValidateMppl2.disableField('optMppl2SubFungsi');
      }
    });

    $('#txtMppl2Dokumen').on('change', function () {
      const preview = document.getElementById('pdfMppl2Preview');
      const file = document.getElementById('txtMppl2Dokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMppl2IsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMppl2IsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
    ...
    $('#btnMppl2Save').on('click', function () {
      if (!formValidateMppl2.validateNow()) {
  ...
  this.add = function () {
    try {
      ...
      formValidateMppl2.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5Mppl2Title').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgProsesPemerolehan2').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5Mppl2Title').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>Maklumat Proses Pemerolehan');
        $('#modalMpgProsesPemerolehan2').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5Mppl2Title').html('Maklumat Proses Pemerolehan');
        $('#modalMpgProsesPemerolehan2').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMppl2JenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMppl2NoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMppl2TajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMppl2File', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMppl2Preview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMppl2IsiDokumen', isiDokumen, 'textarea');
  };
