function ModalErulling() {
  
  const className = 'ModalErulling';
  let self = this;
  let classFrom;
  let id;
  let submitType = '';

  let formValidateMerl;
  this.getValidationDataMerl = function () {
    return [
      {
        field_id: "optMerlJenisDokumen",
        type: "select",
        name: "Jenis Dokumen",
        validator: {
          notEmpty: true
  ...
  this.init = function () {
    formValidateMerl = new MzValidate(false);
    formValidateMerl.registerFields(self.getValidationDataMerl());
    
    $('#btnMerlClose').on('click', function () {
      console.log('close');
      // $('#modalErulling').modal('hide');
      // worker.terminate();
    });
      
    $('#optMerlJenisDokumen').on('change', function () {
      if (this.value == '6' || this.value == '7') {
        //$('.optionKertas').show();
        formValidateMerl.enableField('optMerlCawangan');
        formValidateMerl.enableField('optMerlFungsi');
        formValidateMerl.enableField('optMerlSubFungsi');
      } else {
        //$('.optionKertas').hide();
        formValidateMerl.disableField('optMerlCawangan');
        formValidateMerl.disableField('optMerlFungsi');
        formValidateMerl.disableField('optMerlSubFungsi');
      }
    });

    $('#txtMerlDokumen').on('change', function () {
      const preview = document.getElementById('pdfMerlPreview');
      const file = document.getElementById('txtMerlDokumen').files[0];
    ...
    const showError = (error) => {
      mzSetValue('txtMerlIsiDokumen', null, 'textarea');
      $('#isiDokumenErr').html(document.createTextNode(`Error: ${error.message}`));
      // document.getElementById("txtMerlIsiDokumenErr").appendChild(document.createTextNode(`Error: ${error.message}`));
    };
    ...
    $('#btnMerlSave').on('click', function () {
      if (!formValidateMerl.validateNow()) {
  ...
  this.add = function () {
    try {
      ...
      formValidateMerl.clearValidation();
      //$('.optionKertas').hide();

      ShowLoader();
      setTimeout(function () {
        $('#h5MerlTitle').html('<i class="fa-duotone fa-circle-plus mr-2"></i>Maklumat eRulling');
        $('#modalErulling').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.edit = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MerlTitle').html('<i class="fa-duotone fa-pen-to-square mr-2"></i>Maklumat eRulling');
        $('#modalErulling').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.view = function (_id) {
    try {
      ...
      setTimeout(function () {
        ...
        $('#h5MerlTitle').html('Maklumat eRulling');
        $('#modalErulling').modal({backdrop: 'static', keyboard: false}).scrollTop(0);
        HideLoader();
      }, 200);
    } catch (e) {
  ...
  this.assignValue = function () {
    ...
    mzSetValue('optMerlJenisDokumen', jenisDokumen, 'select');
    mzSetValue('txtMerlNoRujukanFail', 'SPP.600-3/1/1 Jld.21(4)(S)', 'text');
    mzSetValue('txtMerlTajukDokumen', 'KERTAS BIL. KM13/2024: PEMAKLUMAN PERINTAH TAHAN KERJA YANG TERHENTI TERHADAP NAMA PEGAWAI, PEGAWAI PENDIDIKAN PENGAJIAN TINGGI (PPPT) GRED DH54, POLITEKNIK NILAI, NEGERI SEMBILAN [NOMBOR FAIL]', 'textarea');
    mzSetValue('txtMerlFile', '3. MLRTT 672 (Image).pdf', 'text');
    document.getElementById('pdfMerlPreview').src = 'document/3. MLRTT 672 (Image).pdf';
    ...
    mzSetValue('txtMerlIsiDokumen', isiDokumen, 'textarea');
  };
