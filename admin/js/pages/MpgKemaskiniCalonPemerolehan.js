function MpgKemaskiniCalonPemerolehan () {
  
  const className = 'MpgKemaskiniCalonPemerolehan';
  let self = this;
  let dtDisplay;
  ...
  let sectionMpgKemaskiniCalonPemerolehan;
  let userId;
  let dtKcp;
  let formValidate;
  let vData = [
    {
      field_id: "txtKcpmnoPemerolehan",
      type: "text",
      name: "No Pemerolehan",
      validator: {}
    },
    {
      field_id: "txtKcpmSkim",
      type: "text",
      name: "Skim",
      validator: {}
    },
    {
      field_id: "txtKcpmSuruhanjaya",
      type: "text",
      name: "Suruhanjaya",
      validator: {}
    },
    {
      field_id: "txtKcpmTarikhDari",
      type: "text",
      name: "Tarikh Mesyuarat (Dari)",
      validator: {}
    },
    {
      field_id: "txtKcpmTarikhTutup",
      type: "text",
      name: "Tarikh (Tutup)",
      validator: {}
    },
    {
      field_id: "txtKcpmKelompol",
      type: "text",
      name: "Kelompol",
      validator: {}
    },
    {
      field_id: "optKcpmStatus",
      type: "select",
      name: "Status",
      validator: {}
    },
    {
      field_id: "optKcpmProses",
      type: "select",
      name: "Proses",
      validator: {}
    }
  ...
  this.init = function () {
    try {
      ...
      $('.btnAddKcpm').on('click', function () {
        sectionMpgKemaskiniCalonPemerolehan.add(123);
      });
      
      $('#btnKcpmFilter').on('click', function () {
        if (!formValidate.validateNow()) {
          toastr['error'](_ALERT_MSG_VALIDATION, _ALERT_TITLE_ERROR);
        } else {
          dtKcp.search('').columns().search('').draw();
          if ($('#txtKcpmnoPemerolehan').val().length > 0) {
            dtKcp.columns(1).search($('#txtKcpmnoPemerolehan').val() ? $('#txtKcpmnoPemerolehan').val() : '', false, true, false);
          }
          if ($('#txtKcpmSkim').val().length > 0) {
            dtKcp.columns(1).search($('#txtKcpmSkim').val() ? $('#txtKcpmSkim').val() : '', false, true, false);
          }
          if ($('#txtKcpmSuruhanjaya').val().length > 0) {
            dtKcp.columns(1).search($('#txtKcpmSuruhanjaya').val() ? $('#txtKcpmSuruhanjaya').val() : '', false, true, false);
          }
          if ($('#txtKcpmTarikhDari').val().length > 0) {
            moment.locale('ms');
            const tarikhDari = moment($('#txtKcpmTarikhDari').val(), 'DD/MM/YYYY').format('YYYY-MM-DD');
            // dtKcp.columns(6).search($('#txtKcpmTarikhDari').val() ? tarikhDari : '', false, true, false);
            console.log(tarikhDari);
            dtKcp.columns(6).search((d) => d >= tarikhDari);
          }
          if ($('#txtKcpmTarikhTutup').val().length > 0) {
            moment.locale('ms');
            const tarikhDari = moment($('#txtKcpmTarikhTutup').val(), 'DD/MM/YYYY').format('YYYY-MM-DD');
            // dtKcp.columns(6).search($('#txtKcpmTarikhTutup').val() ? tarikhDari : '', false, true, false);
            console.log(tarikhDari);
            dtKcp.columns(6).search((d) => d >= tarikhDari);
          }
          dtKcp.draw();
      ...
      $('#btnKcpmClearFilter').on('click', function () {
        formValidate.clearValidation();
        $('#lblKcpmTarikhDari').addClass('active');
        $('#lblKcpmTarikhHingga').addClass('active');
        dtKcp.search('').columns().search('').draw();
      });
  ...
  this.setSectionMpgKemaskiniCalonPemerolehan = function (_sectionMpgKemaskiniCalonPemerolehan) {
    sectionMpgKemaskiniCalonPemerolehan = _sectionMpgKemaskiniCalonPemerolehan;
  };
